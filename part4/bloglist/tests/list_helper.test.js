...
describe('blogger with most blogs', () => {
  ...
  test('when list has only one blog, equals the author of that blog with' +
    ' blog count of 1', () => {
    const result = listHelper.mostBlogs(listWithOneBlog);
    const expected = { author: 'Robert C. Martin', blogs: 1 };
    assert.deepStrictEqual(result, expected);
  });

  test('of a list with multiple blogs, equals the author with most blogs' +
    ' with blog count calculated correctly', () => {
...
describe('blogger with most likes', () => {
  ...
  test('when list has only one blog, equals the author and likes of that' +
    ...
    const expected = { author: 'Robert C. Martin', likes: 10 };
    assert.deepStrictEqual(result, expected);
  });
